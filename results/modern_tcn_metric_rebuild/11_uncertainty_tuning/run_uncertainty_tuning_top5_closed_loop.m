function result = run_uncertainty_tuning_top5_closed_loop(cfg)
%RUN_UNCERTAINTY_TUNING_TOP5_CLOSED_LOOP Run Top5 tuning candidates on frozen paths.
...
node_root = fullfile(root, 'results', 'modern_tcn_metric_rebuild', ...
    '11_uncertainty_tuning');
top5_root = fullfile(node_root, '04_closed_loop_top5');
manifest_file = fullfile(top5_root, 'top5_closed_loop_manifest.csv');
baseline_root = fullfile(root, 'results', 'modern_tcn_metric_rebuild', ...
    '05_sandbox_closed_loop_if_needed', '03_formal_validation');
closed_root = fullfile(top5_root, '01_closed_loop_runs');
if exist(closed_root, 'dir') ~= 7
    mkdir(closed_root);
end

if exist(manifest_file, 'file') ~= 2
    error('uncertainty_tuning:MissingManifest', ...
        'Missing Top5 manifest: %s', manifest_file);
end
...
if height(candidates) == 0
    error('uncertainty_tuning:NoReadyCandidates', ...
        'No Top5 candidate has passing ONNXRuntime consistency.');
end
...
for p = 1:numel(path_files)
    ...
    path_dir = fullfile(closed_root, path_tag);
    ...
    fprintf('[uncertainty tuning top5] path %d/%d: %s\n', p, numel(path_files), path_tag);
    ...
    compare_tcn_gru_modern_closed_loop_out( ...
        ...
        sprintf('Uncertainty tuning Top5: %s', path_tag), ...
        'uncertainty_tuning_top5');
    ...
    rows(p).summary_file = string(fullfile(path_dir, 'uncertainty_tuning_top5_summary.csv'));
    rows(p).rank_file = string(fullfile(path_dir, 'uncertainty_tuning_top5_rank.csv'));
    rows(p).report_file = string(fullfile(path_dir, 'uncertainty_tuning_top5_report.md'));
    rows(p).status = "ok";
end

path_table = struct2table(rows);
path_runs_file = fullfile(top5_root, 'closed_loop_path_runs.csv');
writetable(path_table, path_runs_file);

result = struct();
result.timestamp = datestr(now, 'yyyy-mm-dd HH:MM:SS');
result.node_root = node_root;
result.top5_root = top5_root;
result.manifest_file = manifest_file;
result.path_table = path_table;
result.path_runs_file = path_runs_file;
result.summary_file = fullfile(top5_root, 'closed_loop_execution_summary.md');
result.mat_file = fullfile(top5_root, 'closed_loop_top5_result.mat');
save(result.mat_file, 'result', '-v7.3');
local_write_summary(result, candidates);

fprintf('[uncertainty tuning top5] path runs: %s\n', result.path_runs_file);
fprintf('[uncertainty tuning top5] summary:   %s\n', result.summary_file);
end
...
if exist(file, 'file') ~= 2 && exist(file, 'file') ~= 4
    error('uncertainty_tuning:MissingFile', 'Missing %s: %s', label, file);
end
end
...
if fid < 0
    warning('uncertainty_tuning:SummaryWriteFailed', ...
        'Cannot write summary: %s', result.summary_file);
    return;
end
cleanup = onCleanup(@() fclose(fid));

fprintf(fid, '# Uncertainty Tuning Top5 Closed-Loop Execution Summary\n\n');
fprintf(fid, '- scope: Top5 closed-loop validation for uncertainty-weighted tuning\n');
...
fprintf(fid, '\n');
fprintf(fid, 'Aggregated decisions are generated by `_run_uncertainty_tuning_v2.py decision`.\n');
end
