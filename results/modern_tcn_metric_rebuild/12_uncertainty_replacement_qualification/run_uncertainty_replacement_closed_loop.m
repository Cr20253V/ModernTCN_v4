function result = run_uncertainty_replacement_closed_loop(cfg)
%RUN_UNCERTAINTY_REPLACEMENT_CLOSED_LOOP Run replacement qualification candidates on frozen paths.
...
init_project();
root = project_root();
node_root = fullfile(root, 'results', 'modern_tcn_metric_rebuild', '12_uncertainty_replacement_qualification');
closed_root = fullfile(node_root, '03_closed_loop_representatives');
manifest_file = fullfile(closed_root, 'replacement_closed_loop_manifest.csv');
...
if exist(manifest_file, 'file') ~= 2
    error('uncertainty_replacement:MissingManifest', ...
        'Missing replacement manifest: %s', manifest_file);
end
...
if height(candidates) == 0
    error('uncertainty_replacement:NoReadyCandidates', ...
        'No replacement candidate has passing ONNXRuntime consistency.');
end
...
for p = 1:numel(path_files)
    ...
    fprintf('[uncertainty replacement] path %d/%d: %s\n', p, numel(path_files), path_tag);
    ...
    compare_tcn_gru_modern_closed_loop_out( ...
        ...
        sprintf('Uncertainty replacement qualification: %s', path_tag), ...
        'uncertainty_replacement');
    ...
    rows(p).summary_file = string(fullfile(path_dir, 'uncertainty_replacement_summary.csv'));
    rows(p).rank_file = string(fullfile(path_dir, 'uncertainty_replacement_rank.csv'));
    rows(p).report_file = string(fullfile(path_dir, 'uncertainty_replacement_report.md'));
    rows(p).status = "ok";
end

path_table = struct2table(rows);
path_runs_file = fullfile(closed_root, 'replacement_closed_loop_path_runs.csv');
...
result.summary_file = fullfile(closed_root, 'replacement_closed_loop_execution_summary.md');
result.mat_file = fullfile(closed_root, 'replacement_closed_loop_result.mat');
save(result.mat_file, 'result', '-v7.3');
local_write_summary(result, candidates);

fprintf('[uncertainty replacement] path runs: %s\n', result.path_runs_file);
fprintf('[uncertainty replacement] summary:   %s\n', result.summary_file);
end
...
if exist(file, 'file') ~= 2 && exist(file, 'file') ~= 4
    error('uncertainty_replacement:MissingFile', 'Missing %s: %s', label, file);
end
end
...
if fid < 0
    warning('uncertainty_replacement:SummaryWriteFailed', ...
        'Cannot write summary: %s', result.summary_file);
    return;
end
cleanup = onCleanup(@() fclose(fid));

fprintf(fid, '# Uncertainty Replacement Closed-Loop Execution Summary\n\n');
fprintf(fid, '- scope: replacement qualification for uncertainty-weighted recipe\n');
...
fprintf(fid, '\n');
fprintf(fid, 'Aggregated decisions are generated by `_run_uncertainty_replacement_qualification.py decision`.\n');
end
