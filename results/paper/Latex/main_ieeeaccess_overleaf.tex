...
% !TeX root = main_ieeeaccess_overleaf.tex
...
\title{Multi-Task ModernTCN Perception-Enhanced LPV-MPC for Path Tracking of a Diagonal Dual-Steer AGV}
...
\includegraphics[width=\linewidth]{fig2_moderntcn_architecture.pdf}

\caption{Architecture of the multi-task ModernTCN scheduling perception module. The input window $Z_k \in \mathbb{R}^{128 \times 19}$ is normalized and passed through the ModernTCN-small encoder (stem, 5 large-kernel depthwise temporal-convolution blocks with pointwise channel mixing and residual connections). The shared representation is formed by concatenating the last-step, mean, and max-pooling readouts with input-window statistics. Three task heads produce the main-condition label $l_{\mathrm{main}}$, the steering-direction label $l_{\mathrm{turn}}$, the confidence $c$, and the slope regression $\hat{\theta}$. The slope output is conditioned by the deployment guard (gain, magnitude limit, rate limit, dead-zone) and fed into the LPV-MPC scheduling vector $\rho = [v, \omega, \theta_{\mathrm{sch}}]$. The classification outputs $l_{\mathrm{main}}$, $l_{\mathrm{turn}}$, and $c$ further shape the MPC constraints and weights through the scheduling update layer.}
