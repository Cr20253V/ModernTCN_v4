...
% =============================
% 文件名：state_eq_ref_train_data.m
% 版本号：V4.6-Train
% 派生自：state_eq_ref.m (V4.x)
% 最后修改时间：2025-11-02
% 作者：LPV-MPC Project
% 
% 【重要】本文件专用于GRU训练数据生成！
% 与原版差异：
%   1. 增加slip_gamma输入（降低摩擦系数μ）
%   2. 增加stall_load输入（外部阻力负载）
%   3. 输入参数扩展为6个
% 
% 同步策略：
%   - 如修改核心物理参数（mass, L, W等），需同步更新原版state_eq_ref.m
%   - 仅注入逻辑（slip_gamma, stall_load）可独立修改
% 
...
%   params: 包含所有车辆参数的结构体
%   slip_gamma: 打滑注入系数 (标量, 可选, 默认=1.0)
%               1.0=正常牵引, 0.3-0.7=打滑工况(降低有效摩擦系数)
%   stall_load: 堵转外部负载[N] (标量, 可选, 默认=0)
%               正值=阻碍运动的外部阻力（如地面障碍）
% 输出参数：
%   x_next: 下一时刻状态向量 (8×1)
% =========================================================================

%% V4.6-Train: 处理可选参数（向后兼容）
if nargin < 5 || isempty(slip_gamma)
    slip_gamma = 1.0;  % 默认值：正常牵引
end
if nargin < 6 || isempty(stall_load)
    stall_load = 0.0;  % 默认值：无外部负载
end
...
mu_base = params.friction_coefficient;   % 基础路面摩擦系数
mu = mu_base * slip_gamma;               % V4.6-Train: 有效摩擦系数（打滑注入时降低）
...
%% === 载荷转移计算 & 滚动阻力计算 ===
[N_lf, N_rf, N_lr, N_rr, F_rolling_total] = compute_load_transfer_train(F_cmd, omega, v, theta_ground, params, mu);
...
% V4.6-Train: 增加stall_load参数
CONT_CORE = @(s, dl, dr) continuous_dynamics_core_v2_train(s, dl, dr, ...
    F_drive_lf, F_drive_rr, F_rolling_total, F_slope, stall_load, m_eff_total, I_z, Lf, Lr, ...
...
%% =========================== 连续动力学核心函数 V2-Train（含slip_gamma和stall_load） ===========================
function ds = continuous_dynamics_core_v2_train(s, delta_lf, delta_rr, Fx_lf, Fx_rr, F_rolling_total, F_slope, ...
    stall_load, m_eff_total, I_z, Lf, Lr, low_speed_thresh, C_af, C_ar, mu, N_lf, N_rr, W, use_smooth, smooth_gain, ...
    rho, CdA)
% 连续动力学核心函数V2-Train - 用于RK4积分的每个子步，支持slip_gamma和stall_load注入
% V4.6-Train 新增参数：
%   stall_load: 外部阻力负载[N]，在纵向动力学中作为额外阻力
...
% 纵向动力学方程（V4.6-Train: 增加stall_load作为额外阻力）
a_x = (F_drive_actual - F_drag - F_slope - stall_load) / max(m_eff_total,1e-6); % 纵向加速度[m/s²]
...
function [N_lf, N_rf, N_lr, N_rr, F_rolling_total] = compute_load_transfer_train(F_cmd, omega, v, theta_g, params, mu)
% 载荷转移计算函数 - V4.6-Train版本（传入mu支持slip_gamma影响）
% 与原版唯一差异：mu作为参数传入，而非从params.friction_coefficient读取
